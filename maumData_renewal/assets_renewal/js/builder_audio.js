...
$(document).ready(function(){
    ...
    $('.aside .btn_expand').on('click', function(){
        ...
        var contentWidth = $('.content').outerWidth(),
            ...
            audioAreaWidth = $('.audioArea').outerWidth();

        if($('.aside').hasClass('on') == true){
            audioAreaWidth = audioAreaWidth - asideWidth;
            $('.audioArea').animate({width: audioAreaWidth}, 300);
        }else{
            audioAreaWidth = contentWidth;
            $('.audioArea').animate({width: audioAreaWidth}, 300);
        }
    });  

    // videoArea width 조절
    function handleAudioAreaWidth(){
        var contentWidth = $('.content').outerWidth(),
            asideWidth = $('.aside').width(),
            audioAreaWidth = $('.audioArea').outerWidth();

        if($('.aside').hasClass('on') == true){
            audioAreaWidth = contentWidth - asideWidth;
            $('.audioArea').css('width', audioAreaWidth);
        }else{
            audioAreaWidth = contentWidth;
            $('.audioArea').css('width', audioAreaWidth);
        }
    }
    handleAudioAreaWidth();
