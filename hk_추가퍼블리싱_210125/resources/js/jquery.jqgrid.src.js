	...
	styleUI : {
		jQueryUI : {
			...
			base : {
				...
				icon_first : "fas fa-angle-double-left",
				icon_prev : "fas fa-angle-left",
				icon_next: "fas fa-angle-right",
				icon_end: "fas fa-angle-double-right",
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			result += " " + ts.p.ariacellprop + "=\""+ts.p.id+"_"+nm+"\"";
			return result + acp[0];
		},
							...
							if(ret[0] === false) {
								...
								if(postdata[oper] === opers.addoper ) {
									...
									if(rp_ge[$t.p.id].reloadAfterSubmit) {
										...
										if($t.p.treeGrid === true) {
											$($t).jqGrid("setTreeRow", postdata[idname],postdata, null, true);
										} else {
											$($t).jqGrid("setRowData", postdata[idname],postdata, null, true);
										}
									}
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			if (o.url === 'clientArray') {
				...
				var k, resp = $($t).jqGrid("setRowData",rowid,tmp, null, true);
				$(ind).attr("editable","0");
					...
					complete: function(res,stat){
						...
						if (stat === "success"){
							...
							if (ret===true) {
								...
								tmp = $.extend({},tmp, tmp2);
								$($t).jqGrid("setRowData",rowid,tmp, null, true);
								$(ind).attr("editable","0");
								for(k=0;k<$t.p.savedRow.length;k++) {
...
$.jgrid.extend({
	...
	// experimental 
	setTreeRow : function(rowid, data, cssp, userowdata) {
		var success=false;
		this.each(function(){
			var t = this;
			if(!t.grid || !t.p.treeGrid) {return;}
			success = $(t).jqGrid("setRowData", rowid, data, cssp, userowdata);
		});
		return success;
	},
