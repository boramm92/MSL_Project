					...
					if(result.success) {
						...
						if(jobInfo != null) {
							...
							if(jobInfo.jobStatus == 'IM') {
								var param = {title: "작업 불가 사유 :", comment: jobInfo.comment};
								var rejectCommentHtml = $.templates("#commentBoxTemplate").render(param);
								$("div.faild_box", $this).prepend(rejectCommentHtml);
							...
							var rejectCommentHtml = $.templates("#commentBoxTemplate").render(
								{title: "반려 사유 :", writeMode:true, comment: jobInfo.rejectComment});
							$("div.reject_box").html(rejectCommentHtml).show();
