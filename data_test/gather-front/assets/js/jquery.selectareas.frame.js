...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateSubDepth = function (visible) {
                if($selection.find('.subDepth').length === 0 && area.subDepth == null){
                    var text = $('label[for="'+$('.type_slt_box').find('.select_group').find('input[type="radio"]:checked').attr('id')+'"]').text();
                    $($selection).append('<h4 class="subDepth" style="font-size: 4px; background-color: lemonchiffon; padding: 0 0 0 0;" >'+text+'</h4>');
                    $('.to-crop').selectAreas('_setSubDepthStatus', area.id, $('.type_slt_box').find('.select_group').find('input[type="radio"]:checked').val());
                }else{
                    $('.to-crop').selectAreas('_setSubDepthStatus', area.id, $('.type_slt_box').find('.select_group').find('input[type="radio"]').find('[value="'+area.subDepth+'"]').val());
                    /*var selectionSubDepth = $selection.find('.subDepth');
                    var sh = $selection.height()-10;
                    var ih = selectionSubDepth.height()-10;
                    var hRatio = Math.abs(Math.round(sh / ih * 10) / 5);
                    selectionSubDepth.css('font-size', hRatio + 'px');*/
                }
            ...
            startSelection  = function (event) {
                ...
                /*if(area.id > 0){
                    $('.to-crop').selectAreas('_clearEmptySubDepthSelection', area.id-1);
                }*/
            ...
            getMousePosition = function (event) {
                ...
                var matrix = $image.parents('.div_img').panzoom('getMatrix');
                var scale = matrix[0];
                var moveX = Number(matrix[4]);
                var moveY = Number(matrix[5]);

                x = (x < 0) ? 0 : (x > ($image.width()*scale)+moveX) ? ($image.width()*scale)+moveX : x;
                y = (y < 0) ? 0 : (y > ($image.height()*scale)+moveY) ? ($image.height()*scale)+moveY : y;
        ...
        $outline = $("<div class=\"select-areas-outline\" data-area-idx='"+id+"'/>")
            .css({
                opacity : options.outlineOpacity,
                position : "absolute",
            });
        $outline = $($outline).insertAfter($trigger);

        // Initialize a selection layer and place it above the outline layer
        $selection = $("<div />")
            .addClass("select-areas-background-area")
            .addClass("text-bind-"+area.id)
            .css({
        ...
        /*var template = '<div class="obj_slt_box common_box">' +
            '<ul class="obj_list">';
        if($image.objArray != undefined && $image.objArray != null && $image.objArray.length > 0){
            $image.objArray.forEach(function(v, i){
                template += '' +
                    '<li>\n' +
                    '    <div class="radio_box">\n' +
                    '        <input type="radio" id="IM_S_TYPE_'+v.code+'_'+id+'_'+i+'" name="IM_S_TYPE_'+id+'" value="'+v.code+'"'+(i === 0 ? ' checked' : '')+' style="appearance:auto;">\n' +
                    '        <label for="IM_S_TYPE_'+v.code+'_'+id+'_'+i+'">'+v.codeName+'</label>\n' +
                    '    </div>\n' +
                    '</li>';
            });
        }else {
        ...
        if($image.objArray != undefined && $image.objArray != null && $image.objArray.length > 0){
            template += '' +
                '<div class="btn_box">\n' +
                '    <button type="button" onclick="$(\'.to-crop\').selectAreas(\'_setSubDepthStatus\', '+id+', $(\'[name=IM_S_TYPE_'+id+']:checked\').val(), $(\'.select-areas-outline[data-area-idx='+id+']\').attr(\'radiocode\'));$(\'.to-crop\').selectAreas(\'_showSubText\','+id+');">완료</button>\n' +
                '</div>';
        }
        template += '</div>';
        $subDepth = $("<div class=\"sub-depth-area\" />")
            .append(template)
            .insertAfter($selection);*/
        /*$('.emptyAreas').remove();*/
        if (options.allowMove) {
    ...
    $.imageSelectAreas.prototype.init = function (object, customOptions) {
        ...
        if(this.options.objArray != null){
            this.$image.objArray = this.options.objArray;
        }
    ...
    $.imageSelectAreas.prototype._clearAreaSelected = function (cb) {
        $.each(this._areas, function (id, area) {
            if (area) {
                area.getData().selected = false;
            }
        });
    };

    $.imageSelectAreas.prototype._clearEmptySubDepthSelection = function (idx) {
        if(this._areas[idx].getData().subDepth == null){
            this.remove(idx);
        }
    };

    $.imageSelectAreas.prototype._setSubDepthStatus = function (i, v) {
        $.each(this._areas, function (id, area) {
            if (area && id == i && v && v !== '') {
                area.getData().subDepth = v;
                area.getData().priority = this.options.priority;
                var contextId = this.options.workData.data['contextId'];
                var self = this;
                if(this.options.workContextId != null){
                    area.getData().workContextId = this.options.workContextId;
                }else{
                    if(contextId){
                        contextId = contextId.filter(function(v){
                            return v.priority === self.options.priority;
                        });
                        if(contextId.length > 0){
                            area.getData().workContextId = contextId[0].contextId;
                        }
                    }
                }
                this.options.workData.data = area.getData();
            }else if(area && id == i){
                this.options.workData.data = area.getData();
            }
        });
    ...
    $.imageSelectAreas.prototype._showSubText = function (id,radioId) {
        var objArray = this.options.objArray;
        var sub = $('.text-bind-'+id);
        var selectSubVal = this._areas[id].getData().subDepth;
        if ($(sub).find('.subDepth').length === 0) {
            if (objArray != null && objArray.length > 0) {
                var text = objArray.find(function (v) {
                    return v.code == selectSubVal;
                }).codeName;
                $(sub).append('<h4 class="subDepth" style="font-size: 4px; background-color: lemonchiffon; padding: 0 0 0 0;" >' + text + '</h4>');
            }
        } else {
            var text = objArray.find(function (v) {
                return v.code == selectSubVal;
            }).codeName;
            $(sub).find('.subDepth').text(text);
        }
    ...
    $.imageSelectAreas.prototype.remove = function (id) {
        if (this._areas[id]) {
            var workData = this.options.workData.data;
            var self = this;
            if(workData[this._areas[id].getData().priority]){
                workData[this._areas[id].getData().priority].forEach(function(v, i){
                    if(v.id && v.id === id){
                        delete workData[self._areas[id].getData().priority][v.id];
                    }
                });
            }
            this._areas[id].deleteSelection();
            this.options.workData.workData = workData;
        }
    };
    ...
    $.imageSelectAreas.prototype.set = function (id, options, silent) {
        if (this._areas[id]) {
            ...
            if(this._areas[id].getData().subDepth != undefined && this._areas[id].getData().subDepth != null && this._areas[id].getData().subDepth != ''){
                this._showSubText(id, $('[name=IM_S_TYPE_'+id+'][value="'+this._areas[id].getData().subDepth+'"]').attr('id'));
                /*$('[name=IM_S_TYPE_'+id+'][value="'+this._areas[id].getData().subDepth+'"]').attr('checked', true);*/
            }
            if(this._areas[id].getData().color != undefined && this._areas[id].getData().color != null && this._areas[id].getData().color != ''){
                $('.select-areas-outline[data-area-idx="'+id+'"]')
                    .css({
                        background: $("input:radio[name=masking_type][id="+this._areas[id].getData().color+"]").val()
                    });
            }
        }
    };
