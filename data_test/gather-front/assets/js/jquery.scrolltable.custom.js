...
$.widget( "ui.scrolltable", {
	...
	_convertTable: function($container){
		...
		var $colgroup = $container.find('>colgroup');
		var $head = $('<table class="' + opts.tableClass + ' st-head-table" cellpadding="0" cellspacing="0" border="0" />').append($colgroup.clone()).append($container.find('>thead')).css('width', '100%');
		var $body = $('<table class="' + opts.tableClass + ' st-body-table" cellpadding="0" cellspacing="0" border="0" />').append($colgroup.clone()).append($container.find('>tbody')).css('width', '100%');
		...
		$container
			...
			.find('.st-body-scroll').css('overflow-y', 'scroll').append($body)
			.find('tr')
