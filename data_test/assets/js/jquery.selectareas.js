...
(function($) {
        ...
        var options = parent.options,
            ...
            updateSubDepth = function (visible) {
                if ($subDepth) {
                    $subDepth.css({
                        display: visible ? "block" : "none",
                        left: area.x + area.width + 10,
                        top: area.y + $subDepth.outerHeight() / 2,
                        "z-index": area.z + 2
                    });
                }
            },
        ...
        if($("input:radio[name=masking_type]").is(":checked")){
            ...
            if($image.objArray != undefined && $image.objArray != null && $image.objArray.length > 0){
                $image.objArray.forEach(function(v, i){
                    template += '<tr>' +
                        '<td><input value="'+v.code+'" id="IM_S_TYPE_'+v.code+'_'+id+'_'+i+'" type="radio" name="IM_S_TYPE_'+id+'" style="appearance:auto;"></td>' +
                        '<td><label for="IM_S_TYPE_'+v.code+'_'+id+'_'+i+'">'+v.codeName+'</label></td>' +
                        '</tr>';
                });
                template += '<tr><td colspan="2" class="s-depth-btn-area"><button type="button" onclick="$(\'.to-crop\').selectAreas(\'_setSubDepthStatus\', '+id+', $(\'[name=IM_S_TYPE_'+id+']:checked\').val(), $(\'.select-areas-outline[data-area-idx='+id+']\').attr(\'radiocode\'));">저장</button></td></tr>';
            }else {
                template += '<tr><td colspan="2">등록된 데이터가 없습니다.</td></tr>'
            }
            template += '</table>';
    ...
    $.imageSelectAreas.prototype._setSubDepthStatus = function (i, v, c) {
        $.each(this._areas, function (id, area) {
            if (area && id == i && v && v !== '') {
                area.getData().subDepth = v;
                area.getData().color = c;
            }
        });
    }
