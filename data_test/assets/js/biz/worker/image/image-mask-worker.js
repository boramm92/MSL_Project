...
//			 $Object.saveContentForWork($answerMap, function () {
	...
	function startCropper(){
		...
		$image.selectAreas({
			...
			objArray: getSubDepthList(),
			allowDelete: false,
		});
		
		$section.find(".div_img").panzoom({
			$zoomIn: $section.find(".fa-plus"),
			$zoomOut: $section.find(".fa-minus"),
			increment: 0.1,
			animate: false,
			disablePan: true
		});
		...
		// get Section Data (TEST)
		$(".getSectionData").click(function(){
			console.log($image.selectAreas('areas'));
		});

		$(".setSectionData").click(function(){
			var arr = [{
				"id": 0,
			...
			$image.selectAreas('add', arr);
		})
	}

	/**
	 * subdepth template render용 data list
	 * @returns {string}
	 */
	function getSubDepthList(){
