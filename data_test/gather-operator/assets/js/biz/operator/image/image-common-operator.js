var ImageJS = (function() {
	...
	function selectUploadFileList(param, callback) {
		...
		MindsJS.loadJson(
			"/oper/image/selectFileList.json",
			param,
			function(result) {
		...
		$.confirm(
			...
			, function() {
				filesElement.uploadFile({
					uri : "/oper/image/fileUpload.json",
					callback : callback,
					projectType : "P",
					showBodyObj : null,
					param : param
	...
	function jobFileConvert(param){
		if(param == null || param.projectId == null) {
			return false;
		}
		$.confirm(
			"파일명을 변환하면 JOB이 생성됩니다. 파일명을 변환하시겠습니까?"
			,function(){
				MindsJS.loadJson(
					"/oper/image/fileConvert.json",
					param,
					function(result) {
						if(result.success) {
							$.alert("파일명 변환이 완료되었습니다.");
						} else {
							$.alert("파일명 변환을 실패하였습니다.");
						}
					},
					null,
					3600000
	...
	function registJobForPreConvertFiles(param, callback) {
		if(param == null || param.projectId == null) {
			return false;
		}
		$.confirm(
			"우선 변환한 파일로 작업을 생성합니다. 진행하시겠습니까?"
			,function(){
				MindsJS.loadJson(
					"/oper/image/registJobByJsonInfo.json",
					param,
					function(result) {
						if(result.success) {
							$.alert("작업등록이 완료 됐습니다.");
	...
	return {
		...
		jobFileConvert: jobFileConvert,
		registJobForPreConvertFiles: registJobForPreConvertFiles
	}
})();