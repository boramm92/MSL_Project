var imageFrameSegmentViewScript4Oper = (function() {
	...
	function bindEventHandler() {
		...
		$("button.btnConvert", $mP).on("click", fileConvert);
		$("button.btnRegistJob", $mP).on("click", registJobPreConvert);
	...
	function selectUploadFileList() {
		...
		param.atchFileType = "M";	// 동영상 파일만 추출 하도록
	...
	function uploadResult(data) {
		if(data.data != null && data.data.length > 0) {
			...
			var html = $.templates("#uploadedFileListTemplate").render(showFileList);
	...
	function fileConvert() {
		var param = mProjectInfo;
		ImageJS.jobFileConvert(param);
	}
	function registJobPreConvert() {
		var param = mProjectInfo;
		ImageJS.registJobForPreConvertFiles(param, selectUploadFileList);
	}
	// 파일삭제
