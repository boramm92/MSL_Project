...
var VoiceJS = (function() {
	...
	function createJobInfo($this, settings) {
		...
		this._jobType = settings._jobType;
		this._jobClassName = settings._jobClassName ? settings._jobClassName : "";
	...
	function selectUploadFileList(param, callback) {
		...
		MindsJS.loadJson(
			"/oper/sound/selectFileList.json",
			param,
			function(result) {
		...
		$.confirm(
			...
			, function() {
				filesElement.uploadFile({
					uri : "/oper/sound/fileUpload.json",
					callback : callback,
					projectType : "S",
					showBodyObj : null,
					param : param
