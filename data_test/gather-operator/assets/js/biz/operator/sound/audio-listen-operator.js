var audioListenViewScript4Oper = (function() {
	var $mP;
	var mProjectInfo = {};  // 프로젝트 기본 정보는 mProjectInfo 객체에서 전역으로 사용한다
	...
	function bindEventHandler() {
		...
		$("button.btnUploadedDel", $mP).on("click", deleteAtchFile);
	}
	function loadData() {
		var param = {};
	...
	function deleteAtchFile() {
		...
		param.jobId = [];
		param.atchFileId = [];
		var checkedList = $(".uploadFileList", $mP).find("input[type=checkbox]:checked");
		checkedList.each(function(i, v) {
			param.jobId.push($(v).siblings("input[name=jobId]").val());
			param.atchFileId.push($(v).siblings("input[name=atchFileId]").val());
		});

		if(param.jobId != null && param.jobId.length > 0) {
			$.confirm(
	...
	function moveProjectState() {
		MindsJS.movePage("/oper/sound/"+mProjectInfo.projectId+"/showProjectStateView.do");
	}
	// 뒤로가기 버튼 눌렀을 때
