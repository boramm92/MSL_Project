var qnaScript = (function() {
	...
	var prefix = '/oper/management';
	var requestApi = {
		selectQnA : prefix + "/selectQnA.json",
		updateQnAReState : prefix + "/updateQnAReState.json",
	...
	function bindEventHandler() {
		...
		$("button.btnMdReState", $mP).off("click").on("click", updateQnAReState);
			...
			, function(result) {
				if(result.success) {
					if(result.data != null) {
						...
						$('select[name=reState]').val(data.reStateCd);
	...
	function updateQnAReState() {
		MindsJS.loadJson(
			requestApi.updateQnAReState
			, { qnaId: qnaId, reState: $('select[name=reState]').val() }
			, function(result) {
				if(result.success) {
					$.alert('상태 저장이 완료되었습니다.');
				} else {
					$.alert('상태 저장 중 오류가 발생하였습니다.');
				}
			}
			, true
		);
