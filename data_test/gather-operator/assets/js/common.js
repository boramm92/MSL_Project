(function ($) {
	...
	$.fn.loadJson = function(uri,callback,timeout) {
		var $this = $(this);
		if(uri == undefined || uri == null) {
			uri = $this.attr("action");
		}
		MindsJS.loadJson(uri, $this.serialize(), callback, timeout);
	};
	...
	$.alert = function(msg, func, title) {
		MindsJS.alert(msg, func, title);
	};
...
var MindsJS = (function() {
	...
	function loadJson(url, paramMap, callback, bActiveIndicator, timeout) {
		if(bActiveIndicator) {
			if(ajaxCallCount == 0) {
				commIndicator.show();
			}
		...
		if(!$.isEmptyObject(timeout)) {
			ajaxParam.timeout = timeout;
		}
		var ctrlRef = $.ajax(ajaxParam);
	}
	...
	function errorHandler(jqXHR, textStatus, errorThrow) {
		if(jqXHR.status == 500) {
			$.alert("시스템 장애로 이용이 불가능합니다. 잠시 후 다시 이용해 주세요.", null, "오류");
		}
		else if(jqXHR.status == 401) {
			location.href = contextPath+"/";
		}
		else if(jqXHR.status == 404) {
			$.alert("페이지를 찾을 수 없습니다.", null, "오류");
		}
...
//			commDialog.lockAlert = true;
			...
			$commDialog.find(".btnOk").on("click", function() {
				//$commDialog.fadeOut();
				$commDialog.hide();
				if(typeof callback === 'function') {
					callback();
...
//		var wordsList = text.split(/\s+/gi);
	...
	function findFile($this, settings) {
		...
		$this.on("change", function() {
			...
			var html = $.templates("#selectUploadFileListTemplate").render(array);
			$showDiv.html(html);
			if(settings.fileCount != null) {
				settings.fileCount.html("(총 " + array.length + "개)");
			}
		});
	}
	...
	function uploadFile($this, settings) {
		...
		for(var i=0; i<fileLength; i++) {
			...
			if(
				...
				&& nameArray[nameArray.length-1].toUpperCase() != "MP4"
