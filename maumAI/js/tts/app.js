...
const AudioStreamPlayer = (function() {
    ...
    function onAudioNodeEnded() {
        ...
        if (abEnded == abCreated) {
            $('.demo_recording2 .demo_infoTxt').text('재생이 완료되었습니다.');
            $('.demo_recording2 .btnBox').show();
            $('.demo_recording2 .recording').hide();
    ...
    function schedulePlayback({channelData, length, numberOfChannels, sampleRate}) {
        ...
        if(abEnded==0){
            $('.demo_recording').hide();
            $('.demo_recording2 .btnBox').hide();
            $('.demo_recording2 .demo_infoTxt').text('음원이 재생 되고 있습니다.');
            $('.demo_recording2 .recording').show();
