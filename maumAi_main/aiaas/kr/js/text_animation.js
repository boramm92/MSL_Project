...
function sectionFigureAni(){
    // 각 수치 롤링 애니메이션 - 숫자
    function numberCounter(targetSection, targetNum){
        this.count = 0; this.diff = 0;
        this.target_count = parseInt(targetNum);
        this.targetSection = document.querySelector(targetSection);
        this.timer = null;
        this.counter();
    };
    ...
    numberCounter.prototype.counter = function(){
        ...
        this.targetSection.innerHTML = this.count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
	...
	function randomChar(type){
		var pool = 'ㄱㄴㄷㄹㅁㅂㅅㅇㅈㅊㅋㅌㅍㅎ가나다라마바사아자차카타파하';	
		var arr = pool.split('');
